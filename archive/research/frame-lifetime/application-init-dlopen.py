...
s.run(['clang','-O2','-Wall','-Wextra','-Werror','-dynamiclib','archive/research/frame-lifetime/application-init-dlopen.c','-o',str(lib)],check=True)
s.run(['clang','archive/research/frame-lifetime/application-init-dlopen-calibrate.c','-o',str(cal)],check=True)
...
s.run(['clang','-O2','-g','-fobjc-arc','-framework','Cocoa','-framework','QuartzCore','-framework','CoreGraphics','-Wl,-sectcreate,__TEXT,__info_plist,archive/research/hello-memory/compat.plist','archive/research/frame-lifetime/external-pages.c','target/frame-lifetime/external-stages.m','-o',str(binary)],check=True)
