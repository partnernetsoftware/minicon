...
`archive/research/frame-lifetime/external-pages.c` reads its own task. Temporarily
...
`archive/research/hello-memory/pixel-host.m`. It retains Regular activation policy,
...
Sources are under the existing exclusive `archive/research/frame-lifetime/` directory:
`external-pages.c`, `attribute-external.py`, `run-external.py`,
`prepare-external-stages.py`, and `run-external-stages.py`.

```sh
clang -dynamiclib -O2 -Wall -Wextra -Werror archive/research/frame-lifetime/external-pages.c -o target/frame-lifetime/external-pages.dylib
EXTERNAL_RUN_OUT=target/frame-lifetime/external-second python3 archive/research/frame-lifetime/run-external.py
EXTERNAL_STAGE_DETAIL=1 python3 archive/research/frame-lifetime/prepare-external-stages.py
clang -O2 -fobjc-arc -framework Cocoa -framework QuartzCore -framework CoreGraphics -Wl,-sectcreate,__TEXT,__info_plist,archive/research/hello-memory/compat.plist archive/research/frame-lifetime/external-pages.c target/frame-lifetime/external-stages.m -o target/frame-lifetime/external-stages
EXTERNAL_STAGE_DETAIL=1 EXTERNAL_STAGE_RUN_OUT=target/frame-lifetime/external-stages-input-split python3 archive/research/frame-lifetime/run-external-stages.py
```
...
`archive/research/frame-lifetime/external-results.json`; raw page-address runs, labels,
...
```sh
python3 archive/research/frame-lifetime/prepare-sample-finish.py
python3 archive/research/frame-lifetime/sample-finish.py
python3 archive/research/frame-lifetime/writing-tools-menu.py
WRITING_TOOLS_TRAITS=1 WRITING_TOOLS_OUT=target/frame-lifetime/writing-tools-traits python3 archive/research/frame-lifetime/writing-tools-menu.py
```
...
Reproducer: `python3 archive/research/frame-lifetime/writing-tools-private.py`.
...
in `archive/research/frame-lifetime/external-results.json`. This one-pair causal
...
in `archive/research/frame-lifetime/external-results.json`. Reproduce only in a
coordinated research window with
`python3 archive/research/frame-lifetime/writing-tools-product.py`; all four GUI
...
`archive/research/frame-lifetime/application-init-notes.md` and
`archive/research/frame-lifetime/application-init-dlopen-status.md`. This investigation
