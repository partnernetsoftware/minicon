...
- [v] each tree tab owns an independent PTY, parser, viewport and failure state
  ...
  client.
- [v] PTY delivery uses a platform-owned fixed 1 MiB byte ring per session
...
- [v] parsing remains bounded to 128 KiB per GUI turn. Reader wakes are
  ...
  turn.
- [v] pointer motion and the cursor-blink timer do not schedule a native
...
- [v] the Windows production graph no longer contains `rmux-pty`. The direct
...
- [v] the primary thread handle from suspended `CreateProcessW` is owned only
...
- [v] Windows process termination keeps its native wait leaf allocation-free:
...
- [v] the child waiter publishes one completion bit directly rather than
  ...
  pipe EOF.
- [v] Windows ConPTY environment inheritance calls `GetEnvironmentStringsW`
...
- [v] Windows environment-block construction uses a PTY-private sorted vector
...
- [v] Windows executable admission compares the exact `.exe`/`.com` PATHEXT leaf
...
- [v] PTY teardown closes bounded output backpressure on the event thread, then
...
- [v] a public multi-tab journey uses `wait-tab-exit` to prove one child can
...
- [v] the pseudoconsole entry points are resolved at run time rather than
...
- [v] the backend is selected by **capability, not by version number**. The
...
- [v] **since 0.1.19 the classic console is the Windows default, and ConPTY is
...
- [v] where ConPTY is absent it is not a choice — the **console agent** is the
...
- [v] the agent argument carries no product name. This adapter is shared with
  `agenterm`, so any product name in it would appear in some other product's
  command line, and a process list is public.
- [v] the difference is sealed below the session contract: both backends use the
  ...
  code branches on it.
- [v] the agent survives the interrupts it raises. `WriteConsoleInput` does not
...
- [v] a resize is recorded by the control thread and applied by the polling
...
- [v] a native Wake services the bounded control queue first and shares one
...
- [v] Windows native key delivery uses one platform `ConsoleGuard` to serialize
...
- [v] Windows physical Left/Right keycodes are normalized at the native pixel
...
- [v] named terminal-key aliases are platform-owned rather than duplicated in
...
- [v] vendored `vt100` emits allocation-free conservative row damage from
...
- [v] the pixel-window contract identifies retained versus transient host
...
- [v] Windows maps typed physical damage to `InvalidateRect` and uses
  ...
  typed failure.
- [v] Windows retained pixel windows deliberately omit `CS_HREDRAW` and
  `CS_VREDRAW`; system expose, typed invalidation and settled geometry redraw
  are the only paint authorities.
- [v] the native ledger times `StretchDIBits` or softbuffer `present` itself
...
- [v] Win32 maps `WM_ENTERSIZEMOVE`/`WM_EXITSIZEMOVE` to shared optional
...
- [v] when capture temporarily renders an inactive target, the platform receives
...
- [v] the Win32 native host keeps `GWLP_USERDATA` behind a reentrancy-checked
...
- [v] the grid face is chosen by **measuring** candidates, not by taking the
...
- [v] cell width is the measured advance, not `tmAveCharWidth` — a font-wide
  ...
  cell width.
- [v] the resolved face is never validated by name. `GetTextFaceW` returns the
...
- [v] selection is memoized per size. `primary_metrics` is on the paint path —
...
- [v] the invariant is asserted, not assumed: the selected face is monospaced at
...
- [v] Windows glyph selection and gray8 coverage execute behind the platform
...
- [v] the Windows GDI leaf maps supplementary scalars through the selected
...
- [v] Windows native glyph faces survive platform raster cache misses at one
...
- [v] shared glyph caching stores its read-heavy bounded FIFO values in a sorted
  contiguous vector, preserving deterministic eviction without retaining
  randomized hashing.

## Architecture kernels

- [v] x86_64 ISA dispatch has one shared authority rather than independent
...
- [v] shared x86 pixel dispatch gives blend and RGB8 pack independent lazy ISA
...
- [v] native geometry does not delegate basic IEEE-754 rounding to CRT math
...
- [v] the 16-step public resize journey dropped from 35 to 18 frames, 17 to 8
  ...
  all 18 requests.
- [v] a paired Windows release probe measured seven idle partial frames at
...
- [v] a post-row-damage Windows release probe produced 33/33 partial raster
...
- [v] a public 100-title OSC stress completed 883/883 direct frames with zero
...
- [v] con's dedicated public sustained-output qualification delivers a fixed
