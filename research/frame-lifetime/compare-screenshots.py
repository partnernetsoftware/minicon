...
for cycle in range(2):
 for variant,binary in artifacts.items():
  ...
  subprocess.run(['python3','research/frame-lifetime/run.py'],env=env,check=True)
...
Path('research/frame-lifetime/screenshot-results.json').write_text(json.dumps(records,indent=2))
