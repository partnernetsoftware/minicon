...
p.write_text(s)
module=p.parent/'cg';module.mkdir(exist_ok=True);shutil.copyfile(root/'research/frame-lifetime/frame_trace.rs',module/'frame_trace.rs')
(root/'research/frame-lifetime/instrumentation.patch').write_text(''.join(difflib.unified_diff(old.splitlines(True),s.splitlines(True),fromfile='a/src/backends/cg.rs',tofile='b/src/backends/cg.rs')))
