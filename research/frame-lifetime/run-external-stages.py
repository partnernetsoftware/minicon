...
try:
 ...
 names+=['complete-menu']
 for name in names:
  result=s.check_output(['python3','research/frame-lifetime/attribute-external.py',str(out/(name+'.tsv')),str(out/'final-vmmap.txt')],text=True);results[name]=json.loads(result)
 ...
 for name,result in results.items():
  if previous:
   ...
   labels=json.loads(s.check_output(['python3','research/frame-lifetime/attribute-external.py',str(newfile),str(out/'final-vmmap.txt')],text=True))
