...
modes=['hello-editable-only','hello-editable-responder','hello-editable']
results=[]
for mode in modes:
 samples=[]
 for repeat in range(3):
  cmd=[str(root/'libc')] if mode=='libc' else [str(root/'probe'),mode]
  p=subprocess.Popen(cmd,stdout=subprocess.PIPE,stderr=open(root/(mode+"-input-metadata.txt"),"w"),text=True)
  try:
 ...
 (root/'input-stages-results.json').write_text(json.dumps(results,indent=2)+'\n')
