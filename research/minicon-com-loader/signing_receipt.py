#!/usr/bin/env python3
"""Verify the exact-byte company-signing boundary for MiniCon artifacts."""
...
EXPECTED_ORGANIZATION = "PARTNERNET SOFTWARE PTY LTD"
...
def subject_has_organization(subject: str, organization: str) -> bool:
    # Public Trust subjects are comma-separated X.500 names.  Avoid accepting
    # the company name merely as a substring of CN or another attribute.
    return any(
        part.strip().casefold() == f"O={organization}".casefold()
        for part in subject.split(",")
    )


def verify_receipt(receipt: dict, root: Path) -> None:
    if receipt.get("schema") != 1 or receipt.get("kind") != "minicon-company-signing":
    ...
    if receipt.get("organization") != EXPECTED_ORGANIZATION:
    ...
    for key, relative in SIGNED_ASSETS.items():
        ...
        if not subject_has_organization(subject, EXPECTED_ORGANIZATION):
            raise ValueError(f"{key}: publisher organization mismatch")
        for field in (
...
def verify(args: argparse.Namespace) -> None:
    ...
    print("PASS company signature + timestamp bound to exact three artifacts")
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        for index, (key, relative) in enumerate(SIGNED_ASSETS.items()):
            ...
            assets[key] = {
                ...
                "signer_subject": f"CN=MiniCon, O={EXPECTED_ORGANIZATION}, C=AU",
        ...
        assets["minicon.com"]["signer_subject"] = f"CN={EXPECTED_ORGANIZATION}, O=OTHER PTY LTD"
        try:
            verify_receipt(receipt, root)
        except ValueError as exc:
            assert "organization mismatch" in str(exc)
        else:
            raise AssertionError("CN-only company name unexpectedly passed")
