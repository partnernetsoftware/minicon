#!/bin/bash
# Research-only: count live PTY threads/handles, tab owners, GDI vs heap
# after extra-tab close. Not an RSS wrapper court. Does not cut PTY capacity.
...
LOG="$LOG_DIR/close-owners-$SOURCE-$STAMP.log"
...
{
  ts "source=$SOURCE"
  ts "note=close-owner sample; not RSS wrapper; hang 33620K is not this row"
} | tee "$LOG"
...
GUEST_PROBE="$WINDOWS_ROOT\\probe-gdi-objects.ps1"
GUEST_SAMPLE="$WINDOWS_ROOT\\sample-close-owners.ps1"
ts "push product+probes" | tee -a "$LOG"
court push "$COURT" "$TARGET_TRIPLE/minicon.exe" "$GUEST_EXE"
court push "$COURT" "$SCRIPT_DIR/probe-gdi-objects.ps1" "$GUEST_PROBE"
court push "$COURT" "$SCRIPT_DIR/sample-close-owners.ps1" "$GUEST_SAMPLE"

runner_tmp="$(mktemp -d)"
trap 'rm -rf "$runner_tmp"; cleanup' EXIT
job_id="win_aarch64_close_owners_$$_$RANDOM"
...
printf '%s\n' \
  ...
  "& '$GUEST_SAMPLE' -Exe '$GUEST_EXE' -Probe '$GUEST_PROBE' -Log '$GUEST_LOG' -Result '$RESULT'" \
  'exit $LASTEXITCODE' | court push "$COURT" - "$JOB"
printf 'ready' | court push "$COURT" - "$READY"
ts "job submitted; poll exit (host bound 120s)" | tee -a "$LOG"
...
if [ -f "$LOG.hostout" ]; then cat "$LOG.hostout" | tee -a "$LOG"; fi
runner_rc="$(tr -d '\r\n' <"$runner_tmp/exit")"
ts "WIN_CLOSE_OWNERS_EXIT=$runner_rc" | tee -a "$LOG"
echo "WIN_CLOSE_OWNERS_EXIT=$runner_rc" | tee -a "$LOG"
exit "$runner_rc"
