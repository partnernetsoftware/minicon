#!/bin/bash
# Research-only: init-phase RSS on one process. IME on. No screenshot.
# Not a counting/wrapper/8-page court.
...
LOG="$LOG_DIR/init-phases-$SOURCE-$STAMP.log"
...
{
  ts "source=$SOURCE"
  ts "note=init-phase causal; IME on; no screenshot; privatized_image != all TIF"
} | tee "$LOG"
...
GUEST_EXE="$WINDOWS_ROOT\\$CELL\\target\\debug\\minicon-release-rss.exe"
GUEST_REGIONS="$WINDOWS_ROOT\\probe-ws-regions.ps1"
GUEST_SAMPLE="$WINDOWS_ROOT\\sample-init-phases.ps1"
...
court push "$COURT" "$SCRIPT_DIR/sample-init-phases.ps1" "$GUEST_SAMPLE"

runner_tmp="$(mktemp -d)"
trap 'rm -rf "$runner_tmp"; cleanup' EXIT
job_id="win_aarch64_init_phases_$$_$RANDOM"
...
printf '%s\n' \
  ...
  "& '$GUEST_SAMPLE' -Exe '$GUEST_EXE' -RegionsProbe '$GUEST_REGIONS' -Log '$GUEST_LOG' -Result '$RESULT'" \
...
if [ -f "$LOG.hostout" ]; then cat "$LOG.hostout" | tee -a "$LOG"; fi
runner_rc="$(tr -d '\r\n' <"$runner_tmp/exit")"
ts "WIN_INIT_PHASES_EXIT=$runner_rc" | tee -a "$LOG"
echo "WIN_INIT_PHASES_EXIT=$runner_rc" | tee -a "$LOG"
exit "$runner_rc"
