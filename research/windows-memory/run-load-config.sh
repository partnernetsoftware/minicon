#!/bin/bash
# Candidate 1: SHGetFolderPathW skip vs keep, activated first-frame steady.
...
LOG="$LOG_DIR/load-config-$SOURCE-$STAMP.log"
...

{
  ...
  ts "note=candidate1 SHGetFolderPathW skip vs keep; activated steady; not APPDATA stand-in; not production pin"
} | tee "$LOG"
...
GUEST_EXE="$WINDOWS_ROOT\\$CELL\\target\\debug\\minicon-load-config.exe"
GUEST_TRACE="$WINDOWS_ROOT\\load-config.trace"
GUEST_SAMPLE="$WINDOWS_ROOT\\sample-load-config.ps1"
ts "push research PE" | tee -a "$LOG"
court push "$COURT" "$PE_DIR/minicon.exe" "$GUEST_EXE"
court push "$COURT" "$SCRIPT_DIR/sample-load-config.ps1" "$GUEST_SAMPLE"

runner_tmp="$(mktemp -d)"
trap 'rm -rf "$runner_tmp"; cleanup' EXIT
job_id="win_aarch64_loadcfg_$$_$RANDOM"
...
runner_rc="$(tr -d '\r\n' <"$runner_tmp/exit")"
ts "WIN_LOAD_CONFIG_EXIT=$runner_rc" | tee -a "$LOG"
echo "WIN_LOAD_CONFIG_EXIT=$runner_rc" | tee -a "$LOG"
exit "$runner_rc"
