#!/bin/bash
# Research-only: two new processes, two client sizes, same frozen PE.
# No screenshot barrier. IME stays on. Not a 32KiB court. Not RSS wrapper.
...
LOG="$LOG_DIR/size-compare-$SOURCE-$STAMP.log"
...
{
  ts "source=$SOURCE"
  ts "pe=$TARGET_TRIPLE/minicon.exe"
  ts "note=size causal compare; IME on; no screenshot; 32KiB unexplained leftover not this court"
} | tee "$LOG"
...
GUEST_SAMPLE="$WINDOWS_ROOT\\sample-size-compare.ps1"
...
court push "$COURT" "$SCRIPT_DIR/sample-size-compare.ps1" "$GUEST_SAMPLE"

runner_tmp="$(mktemp -d)"
trap 'rm -rf "$runner_tmp"; cleanup' EXIT
job_id="win_aarch64_size_compare_$$_$RANDOM"
...
if [ -f "$LOG.hostout" ]; then cat "$LOG.hostout" | tee -a "$LOG"; fi
runner_rc="$(tr -d '\r\n' <"$runner_tmp/exit")"
ts "WIN_SIZE_COMPARE_EXIT=$runner_rc" | tee -a "$LOG"
echo "WIN_SIZE_COMPARE_EXIT=$runner_rc" | tee -a "$LOG"
exit "$runner_rc"
