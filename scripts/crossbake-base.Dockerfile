...
# plus minicon.com (cosmocc, baked into this same base image) -- so that
# build workload can move off GitHub-hosted macOS/Windows runners onto a
# dedicated Linux box?
#
...
# installer so this image can't silently drift from what CI trusts. Sunk
# into the base layer (2026-09-25) rather than left in the thin tail image:
# it never changes as often as the tail's own edits, and every thin-image
# build was re-downloading/re-verifying it (~3.5min) for no reason since it
# was already content-addressed and byte-identical each time.
