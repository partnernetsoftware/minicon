...
/// The 1 MiB ceiling, enforced rather than asserted in prose.
///
/// The size gate did not come across in the repository split — it lived in
/// `agenterm`'s rh pipeline — which left "strictly below 1 MiB" as a
/// documented requirement with nothing checking it, while the product page
/// says the ceiling is enforced. A budget nothing measures is a wish, and a
/// published claim with no gate behind it is worse than no claim.
///
/// Deliberately not a warning threshold: the number is the product's
/// commitment, so the build fails at it.
#[test]
fn the_shipped_executable_stays_under_the_product_budget() {
    /// Strictly below 1 MiB, matching `release_budget_bytes` in the delivery
    /// module: 1 MiB is 1,048,576, so the last acceptable byte is 1,048,575.
    const BUDGET_BYTES: u64 = 1_048_575;

    let path = shipped_binary();
    let bytes = std::fs::metadata(&path)
        .unwrap_or_else(|error| panic!("cannot measure {}: {error}", path.display()))
        .len();
    assert!(
        bytes <= BUDGET_BYTES,
        "{} is {bytes} bytes, over the {BUDGET_BYTES} budget by {}. \
         The ceiling is a product commitment, not a target: either bring the \
         artifact back under it or change the commitment deliberately.",
        path.display(),
        bytes - BUDGET_BYTES
    );
    // A debug build is far over the ceiling and would make this vacuous the
    // other way round; report the headroom so a shrinking margin is visible
